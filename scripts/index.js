...
let financesDifference = finances.reduce((accumulator, finItem, index) => {
  ...
  let diff = finances[index + 1][1] - finances[index][1];
  if (maxDiff === null || maxDiff < diff) {
    maxDiff = diff;
    maxdate = finances[index + 1][0];
  }
  if (minDiff === null || minDiff > diff) {
    minDiff = diff;
    mindate = finances[index + 1][0];
  }
...
Greatest Increase in Profits: ${mindate} $${minDiff}`
);
